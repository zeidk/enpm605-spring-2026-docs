...
.. dropdown:: Click to reveal Multiple Choice answers
   ...
   1. **B** — ``[2, 5, 8]``
      ...
      *Explanation:* ``range(2, 10, 3)`` starts at 2, increments by 3, and stops before 10. Values: 2, 5, 8 (11 would exceed 10).
   ...
   3. **C** — ``None``
      ...
      *Explanation:* ``append()`` modifies the list in-place and returns ``None``. The list is changed, but the return value is ``None``.

   4. **B** — ``dict.get(key)``

      *Explanation:* ``.get()`` returns ``None`` (or a default value) if the key doesn't exist, while ``dict[key]`` raises ``KeyError``.

   5. **B** — ``(1, 2, 3, 4, 5)``

      *Explanation:* The ``+`` operator concatenates tuples, creating a new tuple with all elements.

   6. **B** — Executes if the loop completes without hitting ``break``.

      *Explanation:* The ``else`` clause runs only if the loop exits normally (not via ``break``). Useful for search patterns.

   7. **B** — ``[0, 4, 8]``

      *Explanation:* The comprehension filters for even numbers (0, 2, 4) and doubles them (0, 4, 8).

   8. **B** — ``{2, 3}``

      *Explanation:* ``&`` is the intersection operator, returning elements present in both sets.

   9. **B** — ``t = (42,)``

      *Explanation:* The trailing comma is required for single-element tuples. ``(42)`` is just the integer 42 in parentheses.

   10. **B** — Skips to the next iteration of the loop.

       *Explanation:* ``continue`` immediately starts the next iteration, skipping any remaining code in the current iteration.

   11. **A** — ``["a", "b"]``

       *Explanation:* ``.keys()`` returns a view of dictionary keys, which ``list()`` converts to a list.

   12. **B** — It generates values on demand (lazy evaluation).

       *Explanation:* ``range()`` only stores start, stop, and step (48 bytes total). Values are computed when needed.

   13. **B** — ``[1, [2, 3, 4]]``

       *Explanation:* Shallow copy copies references to nested objects. ``b[1]`` and ``a[1]`` point to the same inner list.

   14. **C** — ``for k, v in d:``

       *Explanation:* Iterating directly over a dictionary yields only keys, not key-value pairs. Use ``.items()`` for pairs.

   15. **C** — ``[]``

       *Explanation:* Slice ``[1:1]`` has start equal to stop, resulting in an empty list.
...
.. dropdown:: Click to reveal True/False answers
   ...
   16. **True**
       ...
       *Explanation:* Lists are mutable. You can modify, add, or remove elements using methods like ``append()``, ``pop()``, or direct index assignment.

   17. **True**

       *Explanation:* Since Python 3.7, dictionaries maintain insertion order as part of the language specification.

   18. **False**

       *Explanation:* ``range(1, 10, 2)`` produces ``[1, 3, 5, 7, 9]``. The value 5 IS in this range, so this is actually **True**.

       *Correction:* This is **True** — 5 is in ``range(1, 10, 2)``.

   19. **True**

       *Explanation:* Tuples are immutable and hashable (if all their elements are hashable), making them valid dictionary keys.

   20. **False**

       *Explanation:* ``enumerate()`` returns an enumerate object (an iterator), not a list. Convert with ``list()`` to get a list of tuples.

   21. **True**

       *Explanation:* ``pop()`` removes an element at a given index (default: last), modifies the list in-place, and returns the removed value.

   22. **False**

       *Explanation:* Set elements must be hashable. Lists are mutable and unhashable, so they cannot be set elements.

   23. **True**

       *Explanation:* Two ranges are equal if they produce the same sequence. ``range(5)`` and ``range(0, 5, 1)`` both produce ``[0, 1, 2, 3, 4]``.

   24. **False**

       *Explanation:* ``a = b`` creates an alias — both names reference the same object. Use ``copy()`` or slicing for an independent copy.

   25. **True**

       *Explanation:* Dictionary comprehensions support ``if`` conditions: ``{k: v for k, v in items if condition}``.
...
.. dropdown:: Click to reveal Essay answer guidelines
   ...
   **Question 26 — Shallow vs Deep Copy**
   ...
   **Question 28 — range() Memory Efficiency**
   ...
   **Question 29 — for vs while Loops**
   ...
   **Question 30 — In-Place vs Out-of-Place Operations**
