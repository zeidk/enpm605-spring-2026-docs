...
.. dropdown:: Click to reveal Multiple Choice answers
   ...
   1. **B** — A module is a single ``.py`` file; a package is a folder containing ``.py`` files with an ``__init__.py``.

      *Explanation:* A module is any single Python file that can be imported. A package is a directory containing one or more modules plus an ``__init__.py`` file (which can be empty) that marks it as a package.

   2. **C** — ``from shape.square import compute_area, compute_perimeter``

      *Explanation:* This approach explicitly imports only the names you need, making it clear where each function comes from and avoiding namespace pollution. Wildcards (``*``) can silently overwrite existing names.

   3. **A** — ``3`` and ``2``

      *Explanation:* ``17 // 5`` is floor division, which returns ``3`` (the quotient rounded down). ``17 % 5`` is the modulus operator, which returns ``2`` (the remainder).

   4. **D** — ``" "`` (string with a single space)

      *Explanation:* A string containing a space is non-empty and therefore truthy. Empty containers (``[]``, ``""``) and zero values (``0``) are falsy.

   5. **B** — ``"default"`` and ``"world"``

      *Explanation:* The ``or`` operator returns the first truthy value (``"default"`` since ``0`` is falsy). The ``and`` operator returns the last value if all are truthy (``"world"``), or the first falsy value otherwise.

   6. **B** — ``if x is None:``

      *Explanation:* The ``is`` operator checks identity (same object in memory). For ``None``, this is the correct approach since there is only one ``None`` object in Python. Using ``==`` would work but is not idiomatic.

   7. **C** — ``"l"``

      *Explanation:* Negative indices count from the end. ``greeting[-1]`` is ``"o"``, so ``greeting[-2]`` is the second-to-last character, ``"l"``.

   8. **B** — ``"ell"``

      *Explanation:* Slicing ``[1:4]`` extracts characters at indices 1, 2, and 3 (stop index is exclusive). From ``"hello"``, these are ``"e"``, ``"l"``, ``"l"``.

   9. **B** — ``"Python"[::-1]``

      *Explanation:* A stride of ``-1`` reverses the string. The result is ``"nohtyP"``.

   10. **A** — ``True``

       *Explanation:* Python supports chained comparisons. ``1 < x < 10`` is equivalent to ``1 < x and x < 10``. Since ``x = 5``, both conditions are true.

   11. **B** — Floats are stored with limited precision, causing rounding errors.

       *Explanation:* Floating-point numbers use IEEE 754 representation, which cannot exactly represent all decimal values. For example, ``0.1 + 0.2 == 0.3`` returns ``False``. Use ``math.isclose()`` instead.

   12. **B** — ``"__main__"``

       *Explanation:* When a script is run directly, Python sets its ``__name__`` variable to ``"__main__"``. When imported as a module, ``__name__`` is set to the module's name.

   13. **C** — ``f"Name: {name}"``

       *Explanation:* F-strings (formatted string literals) are the recommended approach for Python 3.6+. They are faster, more readable, and support inline expressions.

   14. **B** — ``True`` and ``False``

       *Explanation:* ``a == b`` compares values (both lists contain the same elements, so ``True``). ``a is b`` compares identity (they are different objects in memory, so ``False``).

   15. **C** — ``"C"``

       *Explanation:* The conditions are evaluated in order. ``75 >= 90`` is ``False``, ``75 >= 80`` is ``False``, but ``75 >= 70`` is ``True``, so ``grade`` is assigned ``"C"``.
...
.. dropdown:: Click to reveal True/False answers
   ...
   16. **False**
       ...
       *Explanation:* Python uses indentation (whitespace) to define code blocks, not braces. This is a fundamental difference from C-style languages.

   17. **True**

       *Explanation:* An empty list ``[]`` is falsy in Python. The ``not`` operator inverts the Boolean value, so ``not []`` evaluates to ``True``.

   18. **False**

       *Explanation:* Strings in Python are immutable. You cannot change individual characters; instead, you must create a new string. Attempting ``s[0] = "X"`` raises a ``TypeError``.

   19. **True**

       *Explanation:* The ``in`` operator works with strings to check for substrings. For example, ``"ell" in "hello"`` returns ``True``.

   20. **True**

       *Explanation:* Unlike many languages where integers have fixed sizes (32-bit or 64-bit), Python integers can grow to arbitrary size, limited only by available memory.

   21. **False**

       *Explanation:* ``10 // -3`` evaluates to ``-4``, not ``-3``. Floor division rounds toward negative infinity, so ``-3.33...`` rounds down to ``-4``.

   22. **False**

       *Explanation:* Wildcard imports should be avoided because they pollute the namespace and can silently overwrite existing names, making code harder to understand and debug.

   23. **True**

       *Explanation:* This is Python's conditional (ternary) expression syntax. It assigns ``"adult"`` if the condition is true, otherwise ``"minor"``.

   24. **False**

       *Explanation:* ``sys.path.insert()`` only affects the current Python session. The path resets when you start a new Python interpreter. For persistent changes, use ``PYTHONPATH``, ``.pth`` files, or ``pip install -e``.

   25. **True**

       *Explanation:* ``bool`` is indeed a subclass of ``int``. You can verify this with ``isinstance(True, int)`` which returns ``True``. Arithmetic with Booleans is valid: ``True + True == 2``.
...
.. dropdown:: Click to reveal Essay answer guidelines
   ...
   **Question 27 — Short-Circuit Evaluation**
   ...
   **Question 29 — Making Packages Discoverable**
